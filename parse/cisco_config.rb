require 'firewall/CiscoAccessList'
require 'firewall/CiscoRule'

class CiscoConfig < GenericConfig
  ...
  def initialize(config)
    ...
    config.each_line do |line|
      ...
      if line =~ /^ASA Version (.*)$/ then @firmware = $1 end
      ...
      if line =~ /access-list (.*) extended (.*)/ then
        if @access_lists.last == nil
          @access_lists << CiscoAccessList.new($1)
